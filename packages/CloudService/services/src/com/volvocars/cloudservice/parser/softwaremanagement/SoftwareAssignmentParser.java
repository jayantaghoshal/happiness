...
/**
 * XmlParser for parsing Cloud related responses Follows structure found on
...
public final class SoftwareAssignmentParser extends XmlParser {
    ...
    private static SoftwareAssignment ParseSoftwareElement(XmlPullParser parser)
        ...
        while (depth != 0) {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                ...
                } else if (tag.equals("action_request")) {
                    Log.v(LOG_TAG, "Parsing: " + tag);
                    software.actionRequest = ParseActionRequestElement(parser);
                } else {
                    Log.d(LOG_TAG, "Skipping unknown tag: " + tag);
                    SkipElement(parser);
                }
    ...
    private static SoftwareAssignment.ActionRequest ParseActionRequestElement(XmlPullParser parser)
            throws XmlPullParserException, IOException {
        int eventType = parser.getEventType();
        if ((eventType != XmlPullParser.START_TAG) || (!parser.getName().equals("action_request"))) {
            throw new IllegalStateException();
        }
        SoftwareAssignment.ActionRequest actionRequest = new SoftwareAssignment.ActionRequest();
        ...
        while (depth != 0) {
            ...
            if (eventType == XmlPullParser.START_TAG) {
                ...
                if (tag.equals("type")) {
                    Log.v(LOG_TAG, "Parsing: " + tag);
                    actionRequest.type = SoftwareAssignment.ActionRequest.stringToType(ParseString(tag, parser));
                } else if (tag.equals("client_id")) {
                    Log.v(LOG_TAG, "Parsing: " + tag);
                    actionRequest.clientId = ParseString(tag, parser);
                } else if (tag.equals("reason")) {
                    Log.v(LOG_TAG, "Parsing: " + tag);
                    actionRequest.reason = SoftwareAssignment.ActionRequest.stringToReason(ParseString(tag, parser));
                } else if (tag.equals("created")) {
                    Log.v(LOG_TAG, "Parsing: " + tag);
                    actionRequest.created = ParseString(tag, parser);
                } else {
                    Log.d(LOG_TAG, "Skipping unknown tag: " + tag);
                    SkipElement(parser);
                }
            } else if (eventType == XmlPullParser.END_TAG) {
                if (parser.getName().equals("action_request")) {
                    depth--;
                } else {
                    Log.d(LOG_TAG, "Unknown End of Tag: " + parser.getName());
                }
