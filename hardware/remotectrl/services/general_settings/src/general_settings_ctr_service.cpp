...
Return<void> GeneralSettingsCtrlService::registerRemoteCtrlPropertyHandler(
...
Return<void> GeneralSettingsCtrlService::unregisterRemoteCtrlPropertyHandler(
        const ::android::sp<hidl_remotectrl::IRemoteCtrlProperty>& handler) {
    ALOGD("%s", __func__);
...
Return<void> GeneralSettingsCtrlService::sendGetPropertyResp(
...
Return<void> GeneralSettingsCtrlService::sendSetPropertyResp(
        uint32_t requestIdentifier,
        const hidl_remotectrl::RemoteCtrlHalPropertyValue& propValue) {
    ALOGD("%s: (0x%08X)", __func__, requestIdentifier);
...
Return<void> GeneralSettingsCtrlService::notifyPropertyChanged(
        const hidl_remotectrl::RemoteCtrlHalPropertyValue& propValue) {
    ALOGD("%s", __func__);
...
void GeneralSettingsCtrlService::serviceDied(uint64_t cookie,
...
void GeneralSettingsCtrlService::OnStateChange(vsomeip::state_type_e state) {
    if (state == vsomeip::state_type_e::ST_REGISTERED) {
        ALOGD("Conv. API general settings service is now registered");
        {
            std::lock_guard<std::mutex> lock(guard_);
        ...
        ALOGD("Conv. API general settings service is deregistered");
    }
}

bool GeneralSettingsCtrlService::OnMessageReceive(const std::shared_ptr<vsomeip::message>& message) {
    {
        std::lock_guard<std::mutex> lock(guard_);
        if (nullptr == system_service_handler_.get()) {
