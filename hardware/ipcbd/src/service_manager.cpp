...
Return<void> ServiceManager::subscribe(
    uint16_t serviceID,
    ...
    subscribe_cb _hidl_cb)
{
    ...
    if (operationType == OperationType::RESPONSE ||
    ...
    subscriberId = messageDispatcher_.registerMessageCallback(serviceID, operationID, (IpCmdTypes::OperationType) operationType,
...
Return<void> ServiceManager::sendMessage(const Msg& msg, const RetryInfo& retryInfo, sendMessage_cb _hidl_cb)
{
        ...
        (OperationType) msg.pdu.header.operationType == OperationType::NOTIFICATION_REQUEST)
    {
        ...
        _hidl_cb({false,
                  std::string("Message with OperationType " +
                  std::to_string(uint8_t(msg.pdu.header.operationType)) +
...
Return<void> ServiceManager::sendRequest(
    const Msg& msg,
    ...
    sendRequest_cb _hidl_cb)
{
        ...
        (OperationType) msg.pdu.header.operationType != OperationType::NOTIFICATION_REQUEST)
    {
        ALOGE("- Ipcb::sendRequest failed, request with OperationType %d cannot be sent, use sendMessage()",
            uint8_t(msg.pdu.header.operationType));
            _hidl_cb({false,
                std::string("Request with OperationType " +
                std::to_string(uint8_t(msg.pdu.header.operationType)) +
    ...
    //Prepare caller data
    std::shared_ptr<MessageDispatcher::CallerData> caller_data =
        std::make_shared<MessageDispatcher::CallerData>();

    caller_data->responseCallback =
        [this, callbackHandler, caller_data](Message& message){
