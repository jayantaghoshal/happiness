...
UdpSocket::UdpSocket(IDispatcher& dispatcher, EcuIpMap ecu_ip_map) : Socket(dispatcher, AF_INET, SOCK_DGRAM, IPPROTO_UDP, ecu_ip_map)
{
...
void UdpSocket::read(std::vector<uint8_t> &buffer, Message::Ecu &ecu)
{
    ...
    auto it = std::find_if(
        ecu_ip_map_.begin(), ecu_ip_map_.end(), [&fromIp, fromPort](const std::pair<Message::Ecu, EcuAddress> &pair) {
