...
TEST_F(VtsGnssIntegrationTest, recievedOk)
{
  ...
  callback->onLocationCallback=[&promise](const GnssLocation& location)
  {
    ALOGD("Lat: %f Long: %f, time %lu, altitudeMeters %f, speed %f, heading %f", location.latitudeDegrees,
      location.longitudeDegrees, location.timestamp, location.altitudeMeters, location.speedMetersPerSec, location.bearingDegrees);
  ...
  temp_pdu.createHeader(
      (IpCmdTypes::ServiceId)Connectivity::VccIpCmd::ServiceId::Positioning,
