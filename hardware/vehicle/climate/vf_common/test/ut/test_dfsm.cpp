...
class DFSMMainTest : public ::testing::Test {
  ...
  protected:
    ...
    std::unique_ptr<DFsm_Main> createSut() { return std::make_unique<DFsm_Main>(); }
};
...
TEST_F(DFSMMainTest, TransitionToState_WhenNotRegisterInitStateAndChangeState_WillNotBeInNewState) {
    auto sut = createSut();
...
TEST_F(DFSMMainTest, TransitionToState_WhenChangeStateAndNoTransitionCreated_WillNotBeInNewState) {
    auto sut = createSut();
    sut->createState(INIT);
    sut->registerMainInitState(INIT);
...
TEST_F(DFSMMainTest, TransitionToState_WhenRegisterInitStateAndChangeState_WillBeInNewState) {
    auto sut = createSut();
    sut->createState(INIT);
    sut->registerMainInitState(INIT);
...
TEST_F(DFSMMainTest, TransitionToState_WhenHavingRegisteredSubState_WillBeInBothParentAndChildStates) {
    auto sut = createSut();
    sut->createState(INIT);
    sut->registerMainInitState(INIT);
...
TEST_F(DFSMMainTest, TransitionToState_WhenStateNotExist_WillStayInPreviousState) {
    auto sut = createSut();
    sut->createState(INIT);
    sut->registerMainInitState(INIT);
...
TEST_F(DFSMMainTest, Overall_WhenGoingBetweenStates_WillBeInRightParentAndChildStates) {
    auto sut = createSut();
    sut->createState(INIT);
    sut->registerMainInitState(INIT);
...
TEST_F(DFSMMainTest, CallbackBody_WhenStateEntry_Exit_and_BodySet_WillTriggerLambdaCall) {
    auto sut = createSut();
    ...
    sut->createState(
            STATE_2, 0, [this]() { callbackValue_++; }, [this]() { callbackValue_++; }, [this]() { callbackValue_++; });
...
TEST_F(DFSMMainTest, ExecuteTransition_WhenTransitionExistWithPositiveGuard_WillEnterNewState) {
    auto sut = createSut();
...
TEST_F(DFSMMainTest, addStateEntryBodyAndExit_WhenAlreadyExist_WillNotRegisterNew) {
    auto sut = createSut();
...
TEST_F(DFSMMainTest, CreateTransition_WhenStateNotExist_WillNotChangeToNonExistingState) {
    auto sut = createSut();
...
TEST_F(DFSMMainTest, CreateStateEntryBodyAndExit_WhenStateNotExist_WillNotRegisterLamdaFunction) {
    auto sut = createSut();
...
TEST_F(DFSMMainTest, AddInitState_WhenStateNotExist_WillNotChangeState) {
    auto sut = createSut();
...
class DFSMInheranceTest : private DFsm_Main, public ::testing::Test {
  ...
  protected:
    enum TestState { INIT = 0, STATE_1, STATE_2, STATE_3, STATE_4 };
    void registerFSM() {
        createState(INIT);
    ...
    bool isInState(int state) { return inState(state); }
    void gotoToState(int toState) { transitionToState(toState); }
};

TEST_F(DFSMInheranceTest, executeTransition_WhenChangeStateWithinATransition_WillCallQueueTransition) {
    registerFSM();
