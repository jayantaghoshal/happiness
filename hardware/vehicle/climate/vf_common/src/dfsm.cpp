...
StatePointer DFsm_State::create(StatePointer parent,
                                int state,
                                EntryPointer entry,
                                ExitPointer exit,
                                BodyPointer body) {
...
DFsm_Main::DFsm_Main(int initState) : currentState_(nullptr), ongoingTransition_(false) {
...
void DFsm_Main::addInitState(int inState, int initState) {
    ...
    if (nullptr != state1 && nullptr != state2) state1->registerInitState(state2);
}
//==================== //
...
void DFsm_Main::changeState(TransitionPointer transition) {
    ...
    if (nullptr != transition->getState2()->getHistoryPoint()) {
        ...
        while (nullptr != it2) {
            if (nullptr != it2.get()->getEntry()) it2.get()->getEntry().get()->execute();
...
void DFsm_Main::executeTransition(TransitionPointer transition) {
    if (inState(transition->getState1()->getState())) {
        ...
        if (nullptr != transition->getGuard()) {
            if (!transition->getGuard()->execute()) return;
        }
...
void DFsm_Main::executeBody() {
    ...
    if (nullptr != currentState_->getBody()) currentState_->getBody()->execute();
}
//==================== //
void DFsm_Main::executeBody(int state) {
    ...
    do {
        if (it->getState() == state) {
            if (nullptr != it->getBody() && inState(it)) it->getBody()->execute();
        }
        it = it->getParent();
    } while (nullptr != it);
}
...
bool DFsm_Main::inState(StatePointer state) {
    auto it = currentState_;
    if (currentState_ == state) return true;

    while (nullptr != it->getParent()) {
        if (it->getParent() == state) return true;
        it = it->getParent();
    }
    return false;
}
...
bool DFsm_Main::inState(int state) {
    auto it = currentState_;
    if (currentState_.get()->getState() == state) return true;

    while (nullptr != it->getParent()) {
        if (it->getParent().get()->getState() == state) return true;
        it = it->getParent();
    }
    return false;
}
