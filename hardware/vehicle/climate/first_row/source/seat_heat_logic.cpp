...
SeatHeatLogic::SeatHeatLogic(UserLocation                                          userLocation,
...
void SeatHeatLogic::handleAuto()
{
    if ((autoSeatHeatOn_.get().getCurrentState() == UserSelectionGen::StateType::AVAILABLE)
        && (autoSeatHeatOn_.get().getCurrentSelection() == UserSelectionGen::OffOnType::ON)
        && !inDynoMode())
    {
    ...
    else
    {
        ...
        seatHeatgAutCdnSignal_.subscribe([this] {});
