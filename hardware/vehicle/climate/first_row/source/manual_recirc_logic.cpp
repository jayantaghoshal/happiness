...
bool carConfigState()
{
    ...
    try
    {
        const CarConfigParams::CC174_AirQualitySystemType airQualitySystem
            = carconfig::getValue<CarConfigParams::CC174_AirQualitySystemType>();
...
ManualRecircLogic::ManualRecircLogic(ReadOnlyNotifiableProperty<userSelectionGen::OffOnSelection>& RecircTimer,
    ...
    , isActive_{ false }
    , carConfig_{ false }
    , timerDispatcher_{ timerDispatcher_ }
    , timeout_{ util::readLocalConfig<std::chrono::minutes>("LCFG_Manual_Recirc") }
{
    ...
    auto handleSignals = [this] {
        if (signalsOk())
        {
            auto isActive = activationCheck(vehModSts_.get().value().UsgModSts,
                                            vehModSts_.get().value().CarModSts1_,
    ...
    shareAirQualitySensorId = shareAirQualitySensor.subscribe(
        [this](const auto) { this->setStateAndValue(isActive_, shareFanLevelFront.get(), shareMaxDefroster.get()); });

    shareMaxDefrosterId = shareMaxDefroster.subscribe(
        [this](const auto) { this->setStateAndValue(isActive_, shareFanLevelFront.get(), shareMaxDefroster.get()); });

    shareFanLevelFrontId = shareFanLevelFront.subscribe(
        [this](const auto) { this->setStateAndValue(isActive_, shareFanLevelFront.get(), shareMaxDefroster.get()); });
...
void ManualRecircLogic::setStateAndValue(bool                            isActive,
                                         ...
                                         FirstRowGen::MaxDefrosterState  maxDefroster)
{
    ...
    if (isActive)
    {
        ...
        if ((fanLevel == FirstRowGen::FanLevelFrontValue::OFF) || (maxDefroster == FirstRowGen::MaxDefrosterState::ON))
        {
        ...
        else
        {
            ...
            case FirstRowGen::ManualRecircState::ON:
                    ...
                    && (shareRecircTimer.get().getCurrentSelection() == userSelectionGen::OffOnType::ON))
                {
                    ...
                    timerDispatcher_.Start(timeout_,
                                           [this]() { requestManualRecirc(FirstRowGen::ManualRecircRequest::OFF); });
                }
                break;
...
bool ManualRecircLogic::activationCheck(const UsgModSts1 usgMode, const CarModSts1 carMode, const OnOff1 parkingClimate)
{
