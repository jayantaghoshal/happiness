...
bool carConfigOk()
{
    try
    {
        ...
        return carConfigValue == CC175::Electronic_Climate_Control_2_zone
               || carConfigValue == CC175::Electronic_Climate_Control_4_zone
               || carConfigValue == CC175::HVAC_Small_2_zone;
    }
...
TemperatureSyncLogic::TemperatureSyncLogic(NotifiableProperty<FirstRowGen::TemperatureSyncState>& temperatureSync,
    ...
    , subscriptions_{}
{
    ...
    if (carConfigOk_)
    {
        ...
        const auto handlePassengerTempHiLoN = [this](const auto tempHiLoN) {
            std::lock_guard<std::recursive_mutex> lock(mutex_);
            if (temperatureSync_.get() == FirstRowGen::TemperatureSyncState::ON && tempHiLoN != driverTempHiLoN_.get())
            {
                log_debug() << LOG_PREFIX << "Set temp sync to OFF";
                temperatureSync_.set(FirstRowGen::TemperatureSyncState::OFF);
            }
