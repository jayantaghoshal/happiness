...
void VehicleEmulator::doGetConfig(VehicleEmulator::EmulatorMessage& rxMsg, VehicleEmulator::EmulatorMessage& respMsg) {
...
void VehicleEmulator::doSetProperty(VehicleEmulator::EmulatorMessage& rxMsg,
    ...
    VehiclePropValue val = {
            .prop = protoVal.prop(), .areaId = protoVal.area_id(), .timestamp = elapsedRealtimeNano(),
    };
    ...
    if (protoVal.has_bytes_value()) {
        val.value.bytes = std::vector<uint8_t>{protoVal.bytes_value().begin(), protoVal.bytes_value().end()};
    }

    if (protoVal.int32_values_size() > 0) {
        val.value.int32Values = std::vector<int32_t>{protoVal.int32_values().begin(), protoVal.int32_values().end()};
    }

    if (protoVal.int64_values_size() > 0) {
        val.value.int64Values = std::vector<int64_t>{protoVal.int64_values().begin(), protoVal.int64_values().end()};
    }

    if (protoVal.float_values_size() > 0) {
        val.value.floatValues = std::vector<float>{protoVal.float_values().begin(), protoVal.float_values().end()};
    }
...
void VehicleEmulator::populateProtoVehicleConfig(emulator::VehiclePropConfig* protoCfg, const VehiclePropConfig& cfg) {
...
void VehicleEmulator::populateProtoVehiclePropValue(emulator::VehiclePropValue* protoVal, const VehiclePropValue* val) {
