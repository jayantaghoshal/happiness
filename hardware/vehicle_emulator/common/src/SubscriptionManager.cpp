...
#undef LOG_TAG
#define LOG_TAG "auto.vehicle@2.0-impl"
...
bool mergeSubscribeOptions(const SubscribeOptions& oldOpts, const SubscribeOptions& newOpts,
    ...
    if (updatedAreas != kAllSupportedAreas) {
        updatedAreas =
                newOpts.vehicleAreas != kAllSupportedAreas ? updatedAreas | newOpts.vehicleAreas : kAllSupportedAreas;
    }
    ...
    bool updated =
            updatedRate > oldOpts.sampleRate || updatedAreas != oldOpts.vehicleAreas || updatedFlags != oldOpts.flags;
    if (updated) {
        *outResult = oldOpts;
...
bool HalClient::isSubscribed(int32_t propId, int32_t areaId, SubscribeFlags flags) {
    auto it = mSubscriptions.find(propId);
    if (it == mSubscriptions.end()) {
        return false;
    }
    const SubscribeOptions& opts = it->second;
    bool res = (opts.flags & flags) && (opts.vehicleAreas == 0 || areaId == 0 || opts.vehicleAreas & areaId);
    return res;
}
...
StatusCode SubscriptionManager::addOrUpdateSubscription(ClientId clientId, const sp<IVehicleCallback>& callback,
...
std::list<HalClientValues> SubscriptionManager::distributeValuesToClients(
...

    {
        ...
        for (const auto& propValue : propValues) {
            ...
            auto clients = getSubscribedClientsLocked(v->prop, v->areaId, flags);
...
std::list<sp<HalClient>> SubscriptionManager::getSubscribedClients(int32_t propId, int32_t area,
                                                                   SubscribeFlags flags) const {
    MuxGuard g(mLock);
    return getSubscribedClientsLocked(propId, area, flags);
}

std::list<sp<HalClient>> SubscriptionManager::getSubscribedClientsLocked(int32_t propId, int32_t area,
                                                                         SubscribeFlags flags) const {
...
bool SubscriptionManager::updateHalEventSubscriptionLocked(const SubscribeOptions& opts, SubscribeOptions* outUpdated) {
    bool updated = false;
...
sp<HalClient> SubscriptionManager::getOrCreateHalClientLocked(ClientId clientId, const sp<IVehicleCallback>& callback) {
...
void SubscriptionManager::unsubscribe(ClientId clientId, int32_t propId) {
    ...
    if (clientIter == mClients.end()) {
        ...
        if (!isClientSubscribedToOtherProps) {
            ...
            if (!res.isOk()) {
                ALOGW("%s failed to unlink to death, client: %p, err: %s", __func__, client->getCallback().get(),
                      res.description().c_str());
            }
