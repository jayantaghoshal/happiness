...
Obd2SensorStore::BitmaskInVector::BitmaskInVector(size_t numBits) { resize(numBits); }

void Obd2SensorStore::BitmaskInVector::resize(size_t numBits) { mStorage = std::vector<uint8_t>((numBits + 7) / 8, 0); }
...
const std::vector<uint8_t>& Obd2SensorStore::BitmaskInVector::getBitmask() const { return mStorage; }
...
Obd2SensorStore::Obd2SensorStore(size_t numVendorIntegerSensors, size_t numVendorFloatSensors) {
    ...
    const size_t numSystemIntegerSensors = toInt(DiagnosticIntegerSensorIndex::LAST_SYSTEM_INDEX) + 1;
...
const std::vector<int32_t>& Obd2SensorStore::getIntegerSensors() const { return mIntegerSensors; }

const std::vector<float>& Obd2SensorStore::getFloatSensors() const { return mFloatSensors; }

const std::vector<uint8_t>& Obd2SensorStore::getSensorsBitmask() const { return mSensorsBitmask.getBitmask(); }
