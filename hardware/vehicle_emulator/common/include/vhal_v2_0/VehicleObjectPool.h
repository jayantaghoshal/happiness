...
class ObjectPool {
  ...
  private:
    const Deleter<T>& getDeleter() {
        if (!mDeleter.get()) {
            Deleter<T>* d = new Deleter<T>(std::bind(&ObjectPool::recycle, this, std::placeholders::_1));
            mDeleter.reset(d);
        }
        return *mDeleter.get();
    }
...
class VehiclePropValuePool {
  public:
     ...
     *
     */
    VehiclePropValuePool(size_t maxRecyclableVectorSize = 4) : mMaxRecyclableVectorSize(maxRecyclableVectorSize){};
  ...
  private:
    ...
    RecyclableType obtainDisposable(VehiclePropertyType valueType, size_t vectorSize) const;
    ...
    class InternalPool : public ObjectPool<VehiclePropValue> {
      public:
        InternalPool(VehiclePropertyType type, size_t vectorSize) : mPropType(type), mVectorSize(vectorSize) {}

        RecyclableType obtain() { return ObjectPool<VehiclePropValue>::obtain(); }
  ...
  private:
    const Deleter<VehiclePropValue> mDisposableDeleter{[](VehiclePropValue* v) { delete v; }};
