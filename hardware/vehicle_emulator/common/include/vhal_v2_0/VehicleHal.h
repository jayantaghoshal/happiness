...
class VehicleHal {
  public:
    ...
    using HalErrorFunction = std::function<void(StatusCode errorCode, int32_t property, int32_t areaId)>;

    virtual ~VehicleHal() {}

    virtual std::vector<VehiclePropConfig> listProperties() = 0;
    virtual VehiclePropValuePtr get(const VehiclePropValue& requestedPropValue, StatusCode* outStatus) = 0;
     ...
     */
    virtual StatusCode subscribe(int32_t property, int32_t areas, float sampleRate) = 0;
