...
class MockedVehicleHal : public VehicleHal {
  public:
    MockedVehicleHal() { mConfigs.assign(std::begin(kVehicleProperties), std::end(kVehicleProperties)); }
  ...
  private:
    ...
    int64_t makeKey(int32_t prop, int32_t area) const { return (static_cast<int64_t>(prop) << 32) | area; }
...
class VehicleHalManagerTest : public ::testing::Test {
  ...
  public:
    ...
    void invokeGet(const VehiclePropValue& requestedPropValue) {
        ...
        bool called = false;
        manager->get(requestedPropValue,
                     [&refStatus, &refValue, &called](StatusCode status, const VehiclePropValue& value) {
                         refStatus = status;
                         refValue = value;
                         called = true;
                     });
        ASSERT_TRUE(called) << "callback wasn't called for prop: " << hexString(requestedPropValue.prop);
...
TEST_F(VehicleHalManagerTest, getPropConfigs) {
    hidl_vec<int32_t> properties = {toInt(VehicleProperty::HVAC_FAN_SPEED), toInt(VehicleProperty::INFO_MAKE)};
    bool called = false;

    manager->getPropConfigs(properties, [&called](StatusCode status, const hidl_vec<VehiclePropConfig>& c) {
...
TEST_F(VehicleHalManagerTest, getAllPropConfigs) {
    bool called = false;
    manager->getAllPropConfigs([&called](const hidl_vec<VehiclePropConfig>& propConfigs) {
        ASSERT_EQ(arraysize(kVehicleProperties), propConfigs.size());

        for (size_t i = 0; i < propConfigs.size(); i++) {
            ASSERT_EQ(toString(kVehicleProperties[i]), toString(propConfigs[i]));
        }
        called = true;
    });
    ASSERT_TRUE(called);  // Verify callback received.
...
TEST_F(VehicleHalManagerTest, halErrorEvent) {
    ...
    hidl_vec<SubscribeOptions> options = {
            SubscribeOptions{.propId = PROP, .flags = SubscribeFlags::DEFAULT},
    };
...
TEST_F(VehicleHalManagerTest, subscribe) {
    ...
    hidl_vec<SubscribeOptions> options = {SubscribeOptions{.propId = PROP, .flags = SubscribeFlags::DEFAULT}};
    ...
    ASSERT_TRUE(cb->waitForExpectedEvents(0))
            << " Unexpected events received: " << receivedEnvents.size()
            << (receivedEnvents.size() > 0 ? toString(receivedEnvents.front()[0]) : "");
    ...
    ASSERT_TRUE(cb->waitForExpectedEvents(1)) << "Events received: " << receivedEnvents.size();

    ASSERT_EQ(toString(actualValue), toString(cb->getReceivedEvents().front()[0]));
}
...
TEST_F(VehicleHalManagerTest, subscribe_WriteOnly) {
    ...
    hidl_vec<SubscribeOptions> options = {
            SubscribeOptions{.propId = PROP, .flags = SubscribeFlags::HAL_EVENT},
    };
