...
class SubscriptionManagerTest : public ::testing::Test {
  public:
    ...
    void SetUp() override { lastUnsubscribedProperty = -1; }
    ...
    static std::list<sp<IVehicleCallback>> extractCallbacks(const std::list<sp<HalClient>>& clients) {
    ...
    std::list<sp<HalClient>> clientsToProp1() {
        return manager.getSubscribedClients(PROP1, toInt(VehicleAreaZone::ROW_1_LEFT), SubscribeFlags::DEFAULT);
    }

    std::list<sp<HalClient>> clientsToProp2() {
        return manager.getSubscribedClients(PROP2, 0, SubscribeFlags::DEFAULT);
    }
    ...
    void assertOnPropertyUnsubscribedNotCalled() { ASSERT_EQ(-1, lastUnsubscribedProperty); }
...
TEST_F(SubscriptionManagerTest, multipleClients) {
    ...
    ASSERT_EQ(StatusCode::OK, manager.addOrUpdateSubscription(1, cb1, subscrToProp1, &updatedOptions));
    ASSERT_EQ(StatusCode::OK, manager.addOrUpdateSubscription(2, cb2, subscrToProp1, &updatedOptions));

    auto clients = manager.getSubscribedClients(PROP1, toInt(VehicleAreaZone::ROW_1_LEFT), SubscribeFlags::HAL_EVENT);
...
TEST_F(SubscriptionManagerTest, negativeCases) {
    ...
    // Wrong zone
    auto clients = manager.getSubscribedClients(PROP1, toInt(VehicleAreaZone::ROW_2_LEFT), SubscribeFlags::HAL_EVENT);
    ...
    // Wrong flag
    clients = manager.getSubscribedClients(PROP1, toInt(VehicleAreaZone::ROW_1_LEFT), SubscribeFlags::SET_CALL);
    ASSERT_TRUE(clients.empty());
}
...
TEST_F(SubscriptionManagerTest, mulipleSubscriptions) {
    ...
    auto clients = manager.getSubscribedClients(PROP1, toInt(VehicleAreaZone::ROW_1_LEFT), SubscribeFlags::DEFAULT);
    ...
    clients = manager.getSubscribedClients(PROP1, toInt(VehicleAreaZone::ROW_2), SubscribeFlags::DEFAULT);
    ASSERT_ALL_EXISTS({cb1}, extractCallbacks(clients));
}
...
TEST_F(SubscriptionManagerTest, unsubscribe) {
    std::list<SubscribeOptions> updatedOptions;
    ASSERT_EQ(StatusCode::OK, manager.addOrUpdateSubscription(1, cb1, subscrToProp1, &updatedOptions));
    ASSERT_EQ(StatusCode::OK, manager.addOrUpdateSubscription(2, cb2, subscrToProp2, &updatedOptions));
    ASSERT_EQ(StatusCode::OK, manager.addOrUpdateSubscription(3, cb3, subscrToProp1and2, &updatedOptions));
