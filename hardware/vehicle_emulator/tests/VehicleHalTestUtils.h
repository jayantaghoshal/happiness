...
constexpr int32_t kCustomComplexProperty =
        0xbeef | VehiclePropertyGroup::VENDOR | VehiclePropertyType::COMPLEX | VehicleArea::GLOBAL;
...
class MockedVehicleCallback : public IVehicleCallback {
  ...
  public:
    ...
    Return<void> onPropertySet(const VehiclePropValue& /* value */) override { return Return<void>(); }
    ...
    const std::vector<HidlVecOfValues>& getReceivedEvents() { return mReceivedEvents; }
...
inline void assertAllExistsAnyOrder(std::initializer_list<T> expected, const Collection& actual, const char* msg) {
    std::set<T> expectedSet = expected;

    for (auto a : actual) {
        ASSERT_EQ(1u, expectedSet.erase(a)) << msg << "\nContains not unexpected value.\n";
    }
