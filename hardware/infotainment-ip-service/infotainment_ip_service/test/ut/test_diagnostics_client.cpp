...
#include "mocks/mock_iplm_service.h"
...
{
    NiceMock<Connectivity::MockMessageDispatcher> messageDispatcherStub;
    NiceMock<MockIpLmServiceStubImpl> iplm_service;
    ...
    diagnosticsClient.SetIpLinkManagerInstance(&iplm_service);

    EXPECT_CALL(iplm_service, GetEcuAvailibility(Message::Ecu::VCM, _, _))
        .WillOnce(DoAll(SetArgReferee<1>(IpLmServiceStubImpl::ResourceGroup::RG_1),
                        SetArgReferee<2>(IpLmServiceStubImpl::Prio::PRIO_NORM),
                        Return(true)));
    EXPECT_CALL(iplm_service, GetEcuAvailibility(Message::Ecu::TEM, _, _))
        .WillOnce(
            DoAll(SetArgReferee<1>(IpLmServiceStubImpl::ResourceGroup::RG_3 | IpLmServiceStubImpl::ACTION_AVAILABLE),
                  SetArgReferee<2>(IpLmServiceStubImpl::Prio::PRIO_HIGH),
                  Return(true)));
    EXPECT_CALL(diagnosticsClient, setDispatcher(&messageDispatcherStub)).Times(1);
    EXPECT_CALL(diagnosticsClient, RegisterServer(_)).WillOnce(Return(false));
    diagnosticsClient.Initialize(&messageDispatcherStub, common_api_main_loop_context_);
    ...
    std::vector<uint8_t> expected_payload = {
        ...
        static_cast<uint8_t>(IpLmServiceStubImpl::ResourceGroup::RG_1),
        diagnosticsClient.IpLmPrioToDiagnosticsPrio(Connectivity::IpLmServiceStubImpl::Prio::PRIO_NORM),
        static_cast<uint8_t>(diagnosticsClient.MessageEcuToDiagnosticsEcu(Connectivity::Message::Ecu::TEM)),
        static_cast<uint8_t>(IpLmServiceStubImpl::ResourceGroup::RG_3 | IpLmServiceStubImpl::ACTION_AVAILABLE),
        diagnosticsClient.IpLmPrioToDiagnosticsPrio(Connectivity::IpLmServiceStubImpl::Prio::PRIO_HIGH)};
    EXPECT_EQ(g_payload, expected_payload);
}

TEST_F(DiagnosticsClientFixture, IpLmPrioToDiagnosticsPrio)
{
    EXPECT_EQ(diagnosticsClient.IpLmPrioToDiagnosticsPrio(Connectivity::IpLmServiceStubImpl::Prio::PRIO_NORM), 0x01);
    EXPECT_EQ(diagnosticsClient.IpLmPrioToDiagnosticsPrio(Connectivity::IpLmServiceStubImpl::Prio::PRIO_HIGH), 0x02);
    EXPECT_EQ(diagnosticsClient.IpLmPrioToDiagnosticsPrio(static_cast<Connectivity::IpLmServiceStubImpl::Prio>(2)),
              0x00);
}
