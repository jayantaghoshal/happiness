...
static ASN_Result ASN_Stream_EncodeBits(ASN_Stream stream, ASN_BYTE data, U32 bitCount)
{
   ...
   /* "New" byte to process */
   if ( stream->bitPointer == 8 )
      stream->startPtr[stream->bytePointer] = ( ASN_BYTE)0;
...
void * ASN_Malloc( ASN_Session session, U32 size )
{
   ...
   if ( session->malloc_next_ptr + size > session->malloc_end_ptr )
      return (void *)NULL;
...
ASN_BYTE ASN_PrintableString_FindIndexing(ASN_PrintableString ThisPtr, U32 index)
{
   ...
   {
      if (ThisPtr->data[index] == ThisPtr->characterSet[i])
         return (ASN_BYTE) i;
   }
...
{
   if ( (char *)ThisPtr->data == ( char *)NULL)
      return ( char *)NULL;
   if ( ThisPtr->data[ThisPtr->length] == ( ASN_BYTE)'\0' )
      return ( char *)ThisPtr->data;
   else
...
{
   if (stream->bitPointer != 8)
      ASN_Stream_EncodeBits( stream, (ASN_BYTE)0, stream->bitPointer );
}
...
{
   return ( stream->bitPointer == 8 )? stream->bytePointer  :
      stream->bytePointer + 1;                               
}
...
void test_StreamDecodeByte()
{
   ...
   char * sMessage = "";
...
void test_EncodeBits( void )
{
   ...
   if ( result != ASN_RESULT_OK )
      assertEqualsU32( 0, result->error, __LINE__ );
...
void test_EncodeInt( void )
{
   ...
   assertNotEqualsPtr( ASN_RESULT_OK, result, __LINE__ );
   if ( result != ASN_RESULT_OK )
      assertEqualsU32( ASN_ERROR_SIZE_NOT_SUPPORTED, result->error, __LINE__ );
