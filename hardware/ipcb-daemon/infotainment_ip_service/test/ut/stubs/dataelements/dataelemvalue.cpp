...
DataElemValueBase::DataElemValueBase(): _state(State::ERROR), _errorCode(-1) {}

DataElemValueBase::DataElemValueBase(long long timestamp, State state, int errorCode) : _state(state), _errorCode(errorCode) {
    _timestamp = timestamp==-1 ? millisecTimestamp() : timestamp;
}
...
void DataElemValueBase::setError(int errorCode) {_state=State::ERROR;_errorCode=errorCode;} // Only for UT
