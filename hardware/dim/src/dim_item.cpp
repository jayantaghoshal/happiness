...
std::vector<uint8_t> DimItem::GetItemData(VccIpCmd::OperationId fType) const {
    ...
    itemData.insert(itemData.end(),
                    {static_cast<uint8_t>(GetType()),
                     static_cast<uint8_t>(aligment_),
                     static_cast<uint8_t>(truncation_),
                     static_cast<uint8_t>(selectable_)});
    uint32_t size = rawData_.size();
    itemData.insert(itemData.end(),
                    {static_cast<uint8_t>(size >> 24),
                     static_cast<uint8_t>(size >> 16),
                     static_cast<uint8_t>(size >> 8),
                     static_cast<uint8_t>(size)});                      // length of item
