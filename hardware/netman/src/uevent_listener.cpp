  ...
  try {
    auto initial_scan = std::thread([&is_ready, &mutex, &cv] {
      ...
      while (false == cv.wait_for(lk, 50ms, [&] { return is_ready.load(); })) {
        // wait_for reported timeout. this could be because, kernel did not send any uevents and main thread is still
        // blocked on "recvmsg". condition_variable variable will not be signalled. To break this dependency force
        // kernel to generate an event for "lo" interface. If Main thread is ready and is actually listening for events;
