...
Return<void> LscMocker::onResourceGroupStatus(ResourceGroup resourceGroup, ResourceGroupStatus resourceGroupStatus) {
  if(onResourceGroupStatusCallback != nullptr)
    onResourceGroupStatusCallback(resourceGroup,resourceGroupStatus);
  return Void();
}
Return<void> LscMocker::onNodeStatus(Ecu ecuType, bool ecuStatus) {
  if(onNodeStatusCallback != nullptr)
    onNodeStatusCallback(ecuType, ecuStatus);
  return Void();
}