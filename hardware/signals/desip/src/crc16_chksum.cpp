 ...
 *
 * %full_filespec: crc16_chksum.c~1:csrc:bsd_mmm2#4 %
 * @version %version: 1 %
 * @author  %derived_by: lzz760 %
 * @date    %date_modified: Mon Oct 31 12:49:47 2011 %
 *
 *------------------------------------------------------------------------------
 *
 ...
 *
 * The crc16_checkum.c file contains functions for CRC checksum calculation
 *
 ...
 * See crc16_checksum.h for details
 */
uint16_t CRC16_Calculate(const void *start_addr, uint16_t len, uint16_t intial_value)
{
    ...
    /* Copy the intial Value */
    crc=intial_value;

    while(len !=0)
    {
        /* Calculate the Table index */
        table_index = ((crc>>8) ^ *ptr);
        crc = crctable[table_index]^(crc << 8);
        len--;
        ptr++;
    }
    return crc;
...
bool CRC16_Verify(const void *start_addr, uint16_t len, uint16_t intial_value, uint16_t chk_sum)
{
    ...
    crc=CRC16_Calculate(start_addr,len,intial_value);
