...
GEN_VSM_INJECT_SIGNAL_SWITCH_OK = """    case ComConf_ComSignal_is{RTETYPE}_mrx:
    {
        if (sizeof({RTETYPE}) == length) {
            LOGV("Received {RTETYPE} (" << ComConf_ComSignal_is{RTETYPE}_mrx << ")");
            ...
            LOGE("Wrong buffer size received for {RTETYPE} (" << ComConf_ComSignal_is{RTETYPE}_mrx << "). Got " << length << ", expected " << sizeof({RTETYPE}));
        }
    }
    break;
...
GEN_VSM_INJECT_SIGNALGROUP_SWITCH_OK = """    case SignalGroup|ComConf_ComSignalGroup_ig{RTETYPE}_mrx:
    {
        if (sizeof({RTETYPE}) == length) {
            LOGV("Received {RTETYPE} (" << ComConf_ComSignalGroup_ig{RTETYPE}_mrx << ")");
            const {RTETYPE}& rteValue = *static_cast<const {RTETYPE}*>(buffer);
            autosar::{DATAELEM}_info::data_elem_type deValue;
{TRANSFER}
            {DATAELEM}_de->inject(deValue);
        } else {
            LOGE("Wrong buffer size received for {RTETYPE} (" << ComConf_ComSignalGroup_ig{RTETYPE}_mrx << "). Got " << length << ", expected " << sizeof({RTETYPE}));
        }
    }
    break;
