...
class DataElementCommBus : public IDataElementCommBus, mosqpp::mosquittopp {
  public:
    ...
    // void setCallback(autosar::Dir dir, std::function<void(const std::string& name, const QByteArray& payload)>&&
    // newDataElementCallback) override;
...
DataElementCommBus::DataElementCommBus(const std::string brokerAddress) : mosquittopp(initMosquitto().c_str(), false) {
    ...
    if (connectRetCode != MOSQ_ERR_SUCCESS && connectRetCode != MOSQ_ERR_ERRNO) {
        log_warning() << "DataElementCommBus error in connect_async: " << mosquitto_strerror(connectRetCode) << " ("
                      << connectRetCode << ") " << brokerAddress;
        ...
        for (int i = 0; i < 10; i++) {
            ...
            // The return codes MOSQ_ERR_SUCCESS and MOSQ_ERR_ERRNO means that the coonnection will later on be
            // performed async.
            // This is not obvious from the mosquitto documentation but by reading the source code of the mosuitto lib.
            // According to documentation for example MOSQ_ERR_EAI shall never be returned but it happens and in which
            // case
            // the async connection does not work (therfor the re-try).
            if (connectRetCode != MOSQ_ERR_SUCCESS && connectRetCode != MOSQ_ERR_ERRNO) {
                log_warning() << "DataElementCommBus retry-error in connect_async: "
                              << mosquitto_strerror(connectRetCode) << " (" << connectRetCode << ") " << brokerAddress;
            } else {
                break;  // success
            }
        }
    ...
    if (loopStartRetCode != MOSQ_ERR_SUCCESS) {
        log_fatal() << "DataElementCommBus error in loop_start, code: " << mosquitto_strerror(loopStartRetCode) << " ("
                    << loopStartRetCode << ")";
    }
}
...
void DataElementCommBus::send(const std::string& name, const std::string& payload, autosar::Dir dir) {
    ...
    if (retCode != MOSQ_ERR_SUCCESS) {
        log_warn() << "DataElementCommBus::send, publish returned error, code: " << mosquitto_strerror(retCode) << " ("
                   << retCode << "). Name: " << name;
    }
}

void DataElementCommBus::setNewDataElementHandler(
...
void DataElementCommBus::on_disconnect(int rc) {
    if (rc != 0) {
        log_warn() << "DataElementCommBus::on_disconnect, unexpected disconnect, reason: " << mosquitto_strerror(rc)
                   << " (" << rc << ")";
    } else {
        ...
        if (name.length() > 4) {
            ...
            try {
                ...
                log_error() << "unhandled exception in DataElementCommBus::on_message callback,"
                            << " topic: " << message->topic << " error: " << e.what();
            }
        }
    }
}
