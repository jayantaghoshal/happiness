...
template <typename S>
class DESender {
  static_assert(std::is_base_of<autosar::OutTag, S>::value,
                "Only for data elements that are out.");
...
class DataElemValue : public DataElemValueBase {
 public:
  ...
  DataElemValue() {}
  DataElemValue(const value_type& value, long long timestamp, State state,
                int errorCode)
 ...
 public:
  DEReceiver() {
    ...
    if (deReceiverMap().find(_name) == deReceiverMap().end()) {
      ...
      assert(
          "Several DEReceivers for the same signal name is not supported by "
          "this stub");
    }
  }
  ...
  void injectValue(const typename S::data_elem_type& deValue,
                   long long timestamp) {
...
class DEInjector {
 public:
  void inject(const typename S::data_elem_type& deValue,
              long long timestamp = -1) {
    if (DEReceiver<S>::deReceiverMap().find(S::Name()) !=
        DEReceiver<S>::deReceiverMap().end()) {
      DEReceiver<S>::deReceiverMap()[S::Name()]->injectValue(deValue,
                                                             timestamp);
    }
  }
  ...
  void error(int errorCode, long long timestamp = -1) {
    if (DEReceiver<S>::deReceiverMap().find(S::Name()) !=
        DEReceiver<S>::deReceiverMap().end()) {
      DEReceiver<S>::deReceiverMap()[S::Name()]->injectError(errorCode,
                                                             timestamp);
    }
  }
};
