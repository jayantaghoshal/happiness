...
// Inject structs, enums, arrays and errors from the ECD to Application and verify the received values in the
// "Application" (DEReceiver)
...
TEST_F(DETest, stressTest) {
    ...
    for (int i = 0; i < 150000; i++) {
        autosar::UsrSwtDispClimaReqForRowSec d{doubleDist(generator),
                                               true,
                                               1,
                                               true,
    ...
    receiver.subscribeVal([&expectedDataSequence, &lastReceivedIndex, &nrOfReceived, &lastDistance, &receivedIndices,
        ...
        auto comparator = [&value](autosar::UsrSwtDispClimaReqForRowSec const& x) {
            ...
            // const ::testing::internal::FloatingPoint<double> lhs(x.UsrSwtDispClimaReqForTSpForRowSecLe),
            // rhs(value.UsrSwtDispClimaReqForTSpForRowSecLe);
            ...
            return std::fabs(x.UsrSwtDispClimaReqForTSpForRowSecLe - value.UsrSwtDispClimaReqForTSpForRowSecLe) <=
                   0.00001;
        };

        auto it = std::find_if(lastReceivedIndex, std::end(expectedDataSequence), comparator);

        if (it == std::end(expectedDataSequence)) {
            autosar::UsrSwtDispClimaReqForRowSec expectedValue = expectedDataSequence[nrOfReceived];
            std::cout << "ERROR: did not find reveived signal in list of expected values (after previously received): "
                      << std::endl;
            ...
            std::cout << "Expected value: " << std::to_string(expectedValue.UsrSwtDispClimaReqForTSpForRowSecLe)
                      << std::endl;

            std::cout << "Previously received: " << std::endl;
            for (auto& x : receivedIndices) std::cout << "xxx:: " << std::to_string(x) << std::endl;

            auto it2 = std::find_if(std::begin(expectedDataSequence), std::end(expectedDataSequence), comparator);
        ...
        nrOfReceived++;
        if (failed || (static_cast<size_t>(lastDistance + 1) == expectedDataSequence.size()) ||
