  ...
  while (lengthOfCompleteMessage > totalBytesSend) {
    ...
    if (bytesSend <= 0) {
      throw std::runtime_error(
          "Error: SocketConnection::Send, bytesSend < 0 (internal ex: " + std::string(strerror(errno)) + ")");
    }
...
std::string SocketConnection::ReadMessage(int messageLength) {
  ...
  while (totalNumberOfBytesRead < messageLength) {
    ssize_t numberOfBytesRead =
        read(*socket_fd_Ptr_, messageBuffer.data() + totalNumberOfBytesRead, messageLength - totalNumberOfBytesRead);
