...
SocketServer::SocketServer(int port) {
  ...
  if ((*server_socket_fd_Ptr_ = socket(AF_INET, SOCK_STREAM, 0)) == 0) {
    throw std::runtime_error(
        "Error: SocketServer::SocketServer, Socket failed (internal ex: " + std::string(strerror(errno)) + ")");
  }
  ...
  if (bind(*server_socket_fd_Ptr_, (struct sockaddr *)&address_, sizeof(address_)) < 0) {
    throw std::runtime_error(
        "Error: SocketServer::SocketServer, Bind failed (internal ex: " + std::string(strerror(errno)) + ")");
  }

  ALOGV("DEBUG-SocketServer::SocketServer-4");

  if (listen(*server_socket_fd_Ptr_, 3) < 0) {
    throw std::runtime_error("Error: SocketServer::SocketServer, Listen (internal ex: " + std::string(strerror(errno)) +
                             ")");
  }
}
