...
class ARComposition(base.ARObject):
	...
	def updateContent(self, composition):
		...
		other_assembly = set(["_".join([ s.provider_port.text ,s.requester_port.text ]) for k,s in composition.assemblys])
		self_assembly_dict = dict([("_".join([ s.provider_port.text ,s.requester_port.text]),s) for k,s in self.assemblys])
		other_assembly_dict = dict([("_".join([ s.provider_port.text ,s.requester_port.text ]),s) for k,s in composition.assemblys])
