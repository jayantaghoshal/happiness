...
class ARComposition(base.ARObject):
	...
	def __loadDelegations(self):
		...
		for d in del_cons:
			del_name = arxml.find(d,'SHORT-NAME').text
	...
	def __loadAssemblys(self):
		...
		self.assemblys = dict()
		for a in ass_cons:
			ass_name = arxml.find(a, 'SHORT-NAME').text
	...
	def compare(self, composition):
		...
		port_map = [p for k,p in self.ports.iteritems()]
		comp_port_map = [v for i,v in composition.ports.iteritems()]
		...
		return {'added_swcs':added_swcs, 'removed_swcs':removed_swcs, 'added_ports':added_ports, 'removed_ports':removed_ports}
	...
	def updateContent(self, composition):
		...
		self_assembly = set(["_".join([ s.provider_port.text ,s.requester_port.text ]) for k,s in self.assemblys.iteritems()])
		other_assembly = set(["_".join([ s.provider_port.text ,s.requester_port.text ]) for k,s in composition.assemblys.iteritems()])
		self_assembly_dict = dict([("_".join([ s.provider_port.text ,s.requester_port.text]),s) for k,s in self.assemblys.iteritems()])
		other_assembly_dict = dict([("_".join([ s.provider_port.text ,s.requester_port.text ]),s) for k,s in composition.assemblys.iteritems()])
		...
		self_delegation = set(["_".join([s.inner_port.text]) for k,s in self.delegations.iteritems()])
		other_delegation = set(["_".join([s.inner_port.text]) for k,s in composition.delegations.iteritems()])
		self_delegation_dict = dict([("_".join([s.inner_port.text]),s) for k,s in self.delegations.iteritems()])
		other_delegation_dict = dict([("_".join([s.inner_port.text]),s) for k,s in composition.delegations.iteritems()])
	...
	def addPortGroups(self,pg):
		'''
		   ...
		   :return: Nothing
		'''
		for op in arxml.findall(pg,"OUTER-PORTS"):
...
class ARSwc(base.ARObject):
	...
	def __loadServices(self, behavior):
		'''
		   ...
		   :param behavior: Internal behavior of this SWC
		'''
		self.serv_deps = dict([(base.GetShortName(v),ARServiceDependency(v,self.path+base.GetShortName(v),self.arxml)) for v in behavior.findall('.//'+base.add_schema('SWC-SERVICE-DEPENDENCY'))])
	...
	def updateReferences(self, old_path):
		...
		self.__updateReferences(old_path+'/'+self.behavior_path, self.path+'/'+self.behavior_path, self.runnables)
		self.__updateReferences(old_path+'/'+self.behavior_path, self.path+'/'+self.behavior_path, self.parameters)
		self.__updateReferences(old_path+'/'+self.behavior_path, self.path+'/'+self.behavior_path, self.variables)
		self.__updateReferences(old_path+'/'+self.behavior_path, self.path+'/'+self.behavior_path, self.pims)
		self.__updateReferences(old_path+'/'+self.behavior_path, self.path+'/'+self.behavior_path, self.exclusive_areas)
...
class ARPortInterface(base.ARObject):
	...
	def __init__(self, element,path,fileref):
		...
		maps.addReferenceList(element.findall(base.add_schema('OPERATIONS/CLIENT-SERVER-OPERATION/ARGUMENTS/ARGUMENT-DATA-PROTOTYPE/TYPE-TREF')))
...
class ARDelegationConnector(base.ARObject):
	...
	def __init__(self, element):
		base.ARObject.__init__(self, element,None,None)
		self.inner_comp = element.find(base.add_schema('INNER-PORT-IREF/P-PORT-IN-COMPOSITION-INSTANCE-REF/CONTEXT-COMPONENT-REF'))
		self.inner_port = element.find(base.add_schema('INNER-PORT-IREF/P-PORT-IN-COMPOSITION-INSTANCE-REF/TARGET-P-PORT-REF'))
		if self.inner_comp == None:
			self.inner_comp = element.find(base.add_schema('INNER-PORT-IREF/R-PORT-IN-COMPOSITION-INSTANCE-REF/CONTEXT-COMPONENT-REF'))
			self.inner_port = element.find(base.add_schema('INNER-PORT-IREF/R-PORT-IN-COMPOSITION-INSTANCE-REF/TARGET-R-PORT-REF'))
...
class ARSystem(base.ARObject):
	...
	def __loadFibexRefs(self):
		...
		'''
		fibexrefs = self.xmlref.findall(base.add_schema('FIBEX-ELEMENTS/FIBEX-ELEMENT-REF-CONDITIONAL/FIBEX-ELEMENT-REF'))
	...
	def __loadMappings(self):
		...
		self.mapped_datatypes = dict()
		self.mapped_signalsToPort = dict()
		self.mapped_signalsToElement = dict()
		
		sr_signal_mappings = self.xmlref.findall(base.add_schema('MAPPINGS/SYSTEM-MAPPING/DATA-MAPPINGS/SENDER-RECEIVER-TO-SIGNAL-MAPPING'))
		...
		sr_signal_group_mappings = self.xmlref.findall(base.add_schema('MAPPINGS/SYSTEM-MAPPING/DATA-MAPPINGS/SENDER-RECEIVER-TO-SIGNAL-GROUP-MAPPING'))
		...
		for sr in sr_signal_group_mappings:
			...
			# Type mapping
			record_elements = sr.findall(base.add_schema('TYPE-MAPPING/SENDER-REC-RECORD-TYPE-MAPPING/RECORD-ELEMENT-MAPPINGS/SENDER-REC-RECORD-ELEMENT-MAPPING'))
		...
		swc_mapping = self.xmlref.findall(base.add_schema('MAPPINGS/SYSTEM-MAPPING/SW-MAPPINGS/SWC-TO-ECU-MAPPING/COMPONENT-IREFS/COMPONENT-IREF/TARGET-COMPONENT-REF'))
	...
	def setRootComposition(self, new_comp, old_comp):
		...
		root_component = self.xmlref.find(base.add_schema('ROOT-SOFTWARE-COMPOSITIONS/ROOT-SW-COMPOSITION-PROTOTYPE/SHORT-NAME'))
		...
		for s in sr_signal_mappings + sr_signal_group_mappings:
			try:
				map_dict[s.findtext(base.add_schema('DATA-ELEMENT-IREF/CONTEXT-COMPONENT-REF'))].append(s.find(base.add_schema('DATA-ELEMENT-IREF'))) 
			except KeyError:
				map_dict[s.findtext(base.add_schema('DATA-ELEMENT-IREF/CONTEXT-COMPONENT-REF'))] = [s.find(base.add_schema('DATA-ELEMENT-IREF'))]
		...
		for c in new_comp.swcs:
			...
			e.addPropertyElement(base.create_element('CONTEXT-COMPOSITION-REF',value=context_comp, attrib={'DEST':'ROOT-SW-COMPOSITION-PROTOTYPE'}))
			e.addPropertyElement(base.create_element('TARGET-COMPONENT-REF', value=comp_name, attrib={'DEST':'SW-COMPONENT-PROTOTYPE'}))
...
class ARArrayDatatype(ARDatatype):
	...
	def getElementType(self):
		...
		return {'shortname':shortname, 'category':category, 'datatype':datatype, 'max_elems':max_elems, 'semantics':semantics}
...
class ARCompuMethod(base.ARObject):
	...
	def getEnumerations(self):
		...
		return dict([(e.findtext(base.add_schema('COMPU-CONST/VT')), (e.findtext(base.add_schema('LOWER-LIMIT')), e.findtext(base.add_schema('UPPER-LIMIT')))) for e in enumerations])
...
class ARIsignalGroup(base.ARObject):
	...
	def __init__(self, element, path, fileref):
		...
		isignalRefs = element.findall('.//'+base.add_schema('I-SIGNAL-REF')) #self.getSubElem('I-SIGNAL-REFS/I-SIGNAL-REF').text
...
class ARIsignal(base.ARObject):
	...
	def __init__(self, element, path, fileref):
		...
		try:
			self.compumethodref = self.getSubElem('NETWORK-REPRESENTATION-PROPS/SW-DATA-DEF-PROPS-VARIANTS/SW-DATA-DEF-PROPS-CONDITIONAL/COMPU-METHOD-REF').text
		except:
		...
		if 	(self.compumethodref and self.initialvalue) is not None:
			try: # MATS
				...
				if compumethod.category == 'LINEAR':
					self.applinitvalue = str(int(self.initialvalue)*float(compumethod.getCompuNumerator()[1])+float(compumethod.getCompuNumerator()[0]))
				else:
...
class ARServiceDependency(base.ARObject):
	...
	def __init__(self, element, path, fileref):
		...
		self.rolebased_data = {'ramBlock':[], 'defaultValue':[]}
...
class ARGroupMap(base.ARObject):
	...
	def __init__(self, element, path, fileref):
		...
		self.groupsignalmap = dict() # this map is used to map from dataelement member to signal group member (also called group signal)
		if tmpelem is not None: # this is a group/dataelement
			self.rtename = tmpelem.text.split('/')[-1]
			self.isSignal = False
			print "Group: " + self.rtename
			records = self.getAllSubElems2('TYPE-MAPPING/SENDER-REC-RECORD-TYPE-MAPPING/RECORD-ELEMENT-MAPPINGS/SENDER-REC-RECORD-ELEMENT-MAPPING')
			for r in records:
				key=""
				value=""
