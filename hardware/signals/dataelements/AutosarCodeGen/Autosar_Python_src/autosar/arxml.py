...
# LOCAL PARSER
#TODO: Investigate if PIParser is really needed, if not ElementTree can be replaced with cElementTree
...
class ARXml:
	...
	def __init__(self, filename=None):
		if filename is not None:
			self.xml = ET.parse(filename, PIParser())
...
def find_path(element, parent_dict):
	...
	if my_name_elem is not None:
		...
		my_name = ''
	if parent_dict.has_key(element):		
...
def getiterator(element, tag=None, ns=None):
	...
	'''
	if not ns:
		return element.getiterator(tag=base.add_schema(tag))
	else:
		return element.getiterator(tag=tag)
