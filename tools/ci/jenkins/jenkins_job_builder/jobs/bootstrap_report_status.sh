...
get_repos() {
    ...
    (bootstrap_docker_run "repo sync --no-clone-bundle --current-branch -q -j4 vendor/volvocars" || (echo "repo sync failed" && exit 1))
}
clean_all() {
    find . -type f -delete && find . -type d -empty -delete
}
(get_repos || (clean_all && get_repos)) &&
(./vendor/volvocars/tools/ci/jenkins/ci_docker_run.sh python3 vendor/volvocars/tools/ci/shipit/report_job_status.py "${JOB_NAME}" "${BUILD_NUMBER}" "${BUILD_STATUS}" "${GIT_COMMIT}" || (echo "report job status failed" && exit 1)) ||
exit 0 # always green
