...
class ci_database_reporter(abstract_reporter):
    ...
    def plan_finished(self, test_results: List[ResultData]) -> None:
        #InfluxDB:
        self.test_overview_influx(test_results)
    ...
    def test_overview_influx(self, test_results: List[ResultData]) -> None:
        ...
        for r in test_results:
            ...
            for kpi in r.test_kpis:
                try:
                    float(r.test_kpis[kpi])
                    kpidata[kpi] = r.test_kpis[kpi]
                except Exception:
                    kpidata[kpi] = str(r.test_kpis[kpi])
