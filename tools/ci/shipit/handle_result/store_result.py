...
def load_test_results(test: IhuBaseTest,
    ...
    if test_result.save_files_dir:
        with tempfile.TemporaryDirectory() as zip_folder:

            save_file_folder = test_result.save_files_dir
            ...
            if os.path.isdir(save_file_folder):
                for name in os.listdir(save_file_folder):
                    ...
                    uri = "ICUP_ANDROID_CI/%s/%s/%s/%s" % (os.environ["JOB_NAME"], os.environ["BUILD_NUMBER"], testrun_uuid, zip_path)
                    logging.info("Save files to Artifactory uri: {}".format(uri))
                    try:
                        Artifactory().deploy_artifact(uri, os.path.join(zip_folder, zip_path))
                        test_detail["files"][name] = {'location': 'artifactory', 'uri': uri}
                    except Exception:
                        logging.exception("Cannot store files to Artifactory")
