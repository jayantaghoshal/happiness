...
def run(command,
        ...
        cwd: str = None,
        **kwargs) -> Result:
    return ProcessRunner().run(command, encoding, shell, timeout_sec, cwd, **kwargs)
...
def check_output_logged(command,
                        ...
                        cwd: str = None,
                        **kwargs) -> bytes:
    return ProcessRunner().check_output_logged(command, encoding, shell, timeout_sec, cwd, **kwargs)


class ProcessRunner():

    def __init__(self, childloglevel=logging.DEBUG):
        self.childloglevel = childloglevel
        atexit.register(self._close_event_loop)
    ...
    async def _log_and_concat(self,
        ...
        while True:
            ...
            try:
                self.childlogger.log(self.childloglevel, "%s", line_bytes.decode(encoding).strip())  # but we are decoding here
            except UnicodeDecodeError as e:
                logger.error("Failed to decode program output: %s",
                             e.reason)  # TODO: fallback decode to hex or something?
        return output
    ...
    async def _run_logged_helper(self,
        ...
        if shell:
            ...
            s = await asyncio.create_subprocess_exec(command[0], *command[1:],
                                                     ...
                                                     limit=buffer_length,
                                                     **kwargs)
        logger.debug("Executing command: %r, started PID=%d", command, s.pid)
        self.childlogger = logger.getChild("PID%d" % s.pid)
        try:
            stdout, stderr = await asyncio.gather(
                self._log_and_concat(s.stdout, encoding),
                self._log_and_concat(s.stderr, encoding))
        except Exception:
            s.kill()
            raise
        finally:
    ...
    def run(self,
        ...
        r = loop.run_until_complete(
            asyncio.wait_for(
                self._run_logged_helper(command, encoding, shell, cwd=cwd, **kwargs),
                timeout_sec
            ))
        return r

    def check_output_logged(self,
                            command,
                            encoding="utf-8",
                            shell=False,
                            timeout_sec: int=None,
                            cwd: str=None,
                            **kwargs) -> bytes:
        result = self.run(command, encoding=encoding, shell=shell, timeout_sec=timeout_sec, cwd=cwd, **kwargs)
        if result.exitcode != 0:
            logger.debug("PID%d exit with non-0 exitcode: %d", result.pid, result.exitcode)
            raise subprocess.CalledProcessError(result.exitcode, command, result.stdout, result.stderr)
        logger.debug("Process exit successfully")
