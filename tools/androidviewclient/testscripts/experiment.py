...
pil_image = device.takeSnapshot(reconnect=True).convert('RGB')
pil_image.save("New.png", "PNG")
vc.sleep(_s)

pressHomeButton()

#pil_image = device.takeSnapshot(reconnect=True).save("Screen.png", "PNG")
loaded_image = ImageChops.Image.open("Screen.png").convert('RGB')
diff = ImageChops.subtract(pil_image, loaded_image).convert('RGB').getbbox()
print("percent diff")
print(image_diff_percent(loaded_image, pil_image))
print("matching image?")
print(is_equal(loaded_image, pil_image, tolerance=1.0))
#diff.save("Diff.png")
